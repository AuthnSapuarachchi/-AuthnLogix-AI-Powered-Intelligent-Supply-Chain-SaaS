...
import { useProducts, useCreateProduct } from "../../entities/product/model/useProducts";
    ...
    <div className="min-h-screen bg-gray-950 text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8 border-b border-gray-800 pb-4">
          <h1 className="text-3xl font-bold text-blue-500">Inventory Manager</h1>
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {products?.map((p) => (
                      <tr key={p.id} className="hover:bg-gray-800/50">
                        <td className="px-4 py-3 font-mono text-xs text-yellow-500">{p.sku}</td>
                        <td className="px-4 py-3 text-white">{p.name}</td>
                        <td className="px-4 py-3 text-right">{p.price.toFixed(2)}</td>
                        <td className="px-4 py-3 text-right text-blue-400">{p.quantity}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 h-fit">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-gray-300">Target Warehouse</label>
                {loadingWarehouses ? (
