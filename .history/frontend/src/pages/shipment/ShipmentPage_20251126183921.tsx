...
import { useShipments, useCreateShipment } from "../../entities/shipment/model/useShipments";
import { useProducts } from "../../entities/product/model/useProducts"; // Need this for the dropdown
import { CreateShipmentPayload } from "../../entities/shipment/model/types";
...
export const ShipmentPage = () => {
  ...
  // 2. Form
  const { register, handleSubmit, reset, formState: { errors } } = useForm<CreateShipmentPayload>();
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-gray-900 rounded-xl border border-gray-800 p-6">
            ...
            {loadingShipments ? (
              ...
              <div className="overflow-x-auto">
                  ...
                  <tbody className="divide-y divide-gray-800">
                    {shipments?.map((s) => (
                      <tr key={s.id} className="hover:bg-gray-800/50">
                        <td className="px-4 py-3 text-white">{s.product?.name || "Unknown"}</td>
                        <td className="px-4 py-3">{s.destination}</td>
                        <td className="px-4 py-3 text-right font-mono text-yellow-500">-{s.quantity}</td>
                ...
                {shipments?.length === 0 && <p className="text-center py-4 text-gray-500">No shipments yet.</p>}
              </div>
            )}
          </div>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-6 h-fit">
            <h2 className="text-xl font-semibold mb-4">Create Outbound Order</h2>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <Button type="submit" className="w-full" variant="destructive" isLoading={isPending}>
                Confirm Shipment
              </Button>
            </form>
          </div>
