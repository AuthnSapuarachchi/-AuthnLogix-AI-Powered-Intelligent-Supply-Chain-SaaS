...
import {
  useShipments,
  useCreateShipment,
} from "../../entities/shipment/models/useShipments";
import { useProducts } from "../../entities/product/model/useProducts"; // Need this for the dropdown
import type { CreateShipmentPayload } from "../../entities/shipment/models/types";
  ...
  const handleDownloadInvoice = async (id: string) => {
    try {
      const response = await api.get(`/shipments/${id}/invoice`, {
        responseType: "blob", // CRITICAL: Tell Axios this is a binary file
      });
                    ...
                    {shipments?.map((s) => (
                      <tr key={s.id} className="hover:bg-gray-800/50">
                        <td className="px-4 py-3 text-white">
                          ...
                          {s.product?<div className="id"></div>
                        </td>
